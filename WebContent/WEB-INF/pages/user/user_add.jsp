		...
		<div class="mid">
			...
			<div class="right">
				...
				<table style="width: 900px; height: 200px" border="1">
					<thead>
						<tr><td>序号</td><td>选择</td><td>name</td><td>account</td><td>password</td><td>grade</td></tr>
					</thead>
					<c:forEach var="user" items="${list }" varStatus="s">
						<tr><td>${s.index+1 }</td><td>x</td><td>${user.name }</td><td>${user.account }</td><td>${user.pwd }</td><td>${user.grade}</td></tr>
					</c:forEach>
				</table>
			</div>
		</div>
